...
if "XDG_CACHE_HOME" not in os.environ:
    os.environ["XDG_CACHE_HOME"] = os.path.realpath(
...
from tortoise.utils.device import (
    get_device_name,
    set_device_name,
    get_device_count,
    get_device_vram,
...
MODELS["dvae.pth"] = (
    "https://huggingface.co/jbetker/tortoise-tts-v2/resolve/3704aea61678e7e468a06d8eea121dba368a798e/.models/dvae.pth"
)
...
LEARNING_RATE_SCHEMES = {
    "Multistep": "MultiStepLR",
    "Cos. Annealing": "CosineAnnealingLR_Restart",
}
        ...
        def create_voice(self, voice):
            ...
            for file in transcriptions:
                ...
                for segment in result["segments"]:
                    ...
                    entry = (path, segment["end"] - segment["start"], segment["text"])
                ...
                if added == 0:  # added < len(result['segments']):
                    ...
                    entry = (file, end - start, result["text"])
                    candidates.append(entry)
            ...
            # print(bark.__file__)
            bark_location = os.path.dirname(
                os.path.relpath(bark.__file__)
            )  # './modules/bark/bark/'
            output_path = (
                f"./{bark_location}/assets/prompts/" + voice.replace("/", "_") + ".npz"
            )
            np.savez(
                output_path,
                fine_prompt=codes,
        ...
        def inference(self, text, voice, text_temp=0.7, waveform_temp=0.7):
            if voice == "random":
                ...
                if not os.path.exists(
                    "./modules/bark/bark/assets/prompts/" + voice + ".npz"
                ):
            ...
            semantic_tokens = text_to_semantic(
                text, history_prompt=voice, temp=text_temp, silent=False
            )
            ...
            if self.vocos_enabled:
                ...
                wav = self.vocos.decode(
                    features, bandwidth_id=torch.tensor([2], device=self.device)
                )
            else:
...
def generate_bark(**kwargs):
    ...
    if (
        parameters["delimiter"]
        and parameters["delimiter"] != ""
        and parameters["delimiter"] in parameters["text"]
    ):
    ...
    volume_adjust = (
        torchaudio.transforms.Vol(gain=args.output_volume, gain_type="amplitude")
        if args.output_volume != 1
        else None
    )
    ...
    for i, file in enumerate(os.listdir(outdir)):
        ...
        if extension != "json" and extension != "wav":
            continue
        match = re.findall(
            rf"^{cleanup_voice_name(voice)}_(\d+)(?:.+?)?{extension}$", filename
        )
    ...
    for line, cut_text in enumerate(texts):
        ...
        if match and len(match) > 0:
            ...
            try:
                ...
                raise Exception(
                    "Prompt settings editing requested, but received invalid JSON"
                )
        ...
        for j, g in enumerate(gen):
            ...
            if tts.vocos_enabled:
                torchaudio.save(
                    f"{outdir}/{cleanup_voice_name(voice)}_{name}.wav", wav.cpu(), sr
                )
            else:
                write_wav(f"{outdir}/{cleanup_voice_name(voice)}_{name}.wav", sr, wav)
            wav, sr = torchaudio.load(
                f"{outdir}/{cleanup_voice_name(voice)}_{name}.wav"
            )
            ...
            audio_cache[name] = {
                ...
                "settings": get_info(
                    ...
                    if override and "voice" in override
                    else voice,
                    settings=settings,
                ),
            }
    ...
    for k in audio_cache:
        ...
        torchaudio.save(
            f"{outdir}/{cleanup_voice_name(voice)}_{k}.wav",
            audio,
    ...
    for name in audio_cache:
        ...
        if not args.embed_output_metadata:
            with open(
                f"{outdir}/{cleanup_voice_name(voice)}_{name}.json",
                "w",
                encoding="utf-8",
            ) as f:
    ...
    stats = [[parameters["seed"], "{:.3f}".format(info["time"])]]
...
def generate_valle(**kwargs):
    ...
    def fetch_voice(voice):
        ...
        if os.path.isdir(f"./training/{voice}/audio/"):
            files = get_voice(
                name="audio", dir=f"./training/{voice}/", load_latents=False
            )
        else:
    ...
    def get_settings(override=None):
        ...
        )  # [ fetch_voice(voice=selected_voice) for _ in range(3) ]
        return settings
...
def generate_tortoise(**kwargs):
    ...
    def fetch_voice(voice):
        ...
        if voice == "microphone":
            if parameters["mic_audio"] is None:
                raise Exception(
                    "Please provide audio from mic when choosing `microphone` as a voice input"
                )
            voice_samples, conditioning_latents = (
                [load_audio(parameters["mic_audio"], tts.input_sample_rate)],
                None,
            )
        elif voice == "random":
            voice_samples, conditioning_latents = (
    ...
    def get_settings(override=None):
        ...
        if settings["autoregressive_model"] is not None:
            if settings["autoregressive_model"] == "auto":
                settings["autoregressive_model"] = deduce_autoregressive_model(
                    selected_voice
                )
        ...
        settings["voice_samples"], settings["conditioning_latents"], _ = fetch_voice(
            voice=selected_voice
        )
        ...
        if (
            settings["conditioning_latents"] is not None
            and len(settings["conditioning_latents"]) == 2
            and settings["cvvp_amount"] > 0
        ):
            print(
    ...
    def get_info(voice, settings=None, latents=True):
        ...
        if latents and "latents" not in info:
            voice = info["voice"]
            model_hash = (
                settings["model_hash"][:8]
                if settings is not None and "model_hash" in settings
                else tts.autoregressive_model_hash[:8]
            )
            ...
            if voice == "random" or voice == "microphone":
                if args.use_hifigan:
                    if (
                        ...
                        and torch.any(settings["conditioning_latents"])
                    ):
                    ...
                    if (
                        latents
                        and settings is not None
                        and settings["conditioning_latents"]
                    ):
                        os.makedirs(dir, exist_ok=True)
                        torch.save(conditioning_latents, latents_path)
    ...
    for line, cut_text in enumerate(texts):
        ...
        try:
            if args.use_hifigan:
                ...
                filtered_settings = {
                    k: v for k, v in settings.items() if k not in unused_args
                }
    ...
    if args.use_rvc:
        ...
        rvc_index_path = os.path.join(
            "models", "rvc_models", rvc_settings["file_index"]
        )
...
def update_baseline_for_latents_chunks(voice):
    ...
    return (
        int(total_duration / args.autocalculate_voice_chunk_duration_size)
        if total_duration > 0
        else 1
    )
...
def compute_latents(
    ...
    conditioning_latents = tts.get_conditioning_latents(
        voice_samples,
        return_mels=not args.latents_lean_and_mean,
...
class TrainingState:
    def __init__(self, config_path, keep_x_past_checkpoints=0, start=True):
        ...
        self.json_config = json.load(
            open(f"{self.training_dir}/train.json", "r", encoding="utf-8")
        )
    ...
    def spawn_process(self, config_path, gpus=1):
        if args.tts_backend == "vall-e":
            ...
            self.cmd = (
                ["train.bat", config_path]
                if os.name == "nt"
                else ["./train.sh", config_path]
            )
    ...
    def parse_metrics(self, data):
        ...
        if "iteration_rate" in self.info:
            ...
            self.it_rate = (
                f'{"{:.3f}".format(1/it_rate)}it/s'
                if 0 < it_rate and it_rate < 1
                else f'{"{:.3f}".format(it_rate)}s/it'
            )
            ...
            if self.it_rates > 0 and self.it * self.steps > 0:
                ...
                self.epoch_rate = (
                    f'{"{:.3f}".format(1/epoch_rate)}epoch/s'
                    if 0 < epoch_rate and epoch_rate < 1
                    else f'{"{:.3f}".format(epoch_rate)}s/epoch'
                )
        ...
        if self.it > 0:
            ...
            for k in keys["lrs"]:
                ...
                self.statistics["lr"].append(
                    {"epoch": epoch, "it": self.it, "value": self.info[k], "type": k}
                )
            ...
            for k in keys["grad_norms"]:
                if k not in self.info:
                    continue
                self.statistics["grad_norm"].append(
                    {"epoch": epoch, "it": self.it, "value": self.info[k], "type": k}
                )
    ...
    def get_status(self):
        ...
        if len(self.losses) > 0:
            self.metrics["loss"].append(
                f'Loss: {"{:.3f}".format(self.losses[-1]["value"])}'
            )
        ...
        if False and len(self.losses) >= 2:
            ...
            accum_length = (
                len(self.losses) // 2
            )  # i *guess* this is fine when you think about it
            ...
            if deriv != 0:  # dloss < 0:
                ...
                if next_milestone:
                    ...
                    if est_its >= 0:
                        self.metrics["loss"].append(
                            f"Est. milestone {next_milestone} in: {int(est_its)}its"
                        )
                else:
                    est_loss = inst_deriv * (self.its - self.it) + loss_value
                    if est_loss >= 0:
                        self.metrics["loss"].append(
                            f'Est. final loss: {"{:.3f}".format(est_loss)}'
                        )
    ...
    def load_statistics(self, update=False):
        ...
        if args.tts_backend == "tortoise":
            logs = sorted(
                [
                    f"{self.training_dir}/finetune/{d}"
                    for d in os.listdir(f"{self.training_dir}/finetune/")
                    if d[-4:] == ".log"
                ]
            )
        else:
            ...
            logs = sorted(
                [
                    f"{self.training_dir}/{log_dir}/{d}/log.txt"
                    for d in os.listdir(f"{self.training_dir}/{log_dir}/")
                ]
            )
        ...
        for log in logs:
            ...
            for line in lines:
                ...
                if args.tts_backend == "vall-e":
                    if (
                        not averager
                        or averager["key"] != f"{it}_{name}"
                        or averager["mode"] != mode
                    ):
                        averager = {
                            "key": f"{it}_{name}",
                            "name": name,
    ...
    def cleanup_old(self, keep=2):
        ...
        models = sorted(
            [
                int(d[:-8])
                for d in os.listdir(f"{self.training_dir}/finetune/models/")
                if d[-8:] == "_gpt.pth"
            ]
        )
        states = sorted(
            [
                int(d[:-6])
                for d in os.listdir(f"{self.training_dir}/finetune/training_state/")
                if d[-6:] == ".state"
            ]
        )
...
def run_training(
    ...
    training_state = TrainingState(
        config_path=config_path, keep_x_past_checkpoints=keep_x_past_checkpoints
    )
    ...
    for line in iter(training_state.process.stdout.readline, ""):
        ...
        result, percent, message = training_state.parse(
            line=line,
            verbose=verbose,
...
def update_training_dataplot(
    x_min=None, x_max=None, y_min=None, y_max=None, config_path=None
):
    global training_state
    losses = None
    lrs = None
...
def reconnect_training(verbose=False, progress=gr.Progress(track_tqdm=True)):
    ...
    for line in iter(training_state.process.stdout.readline, ""):
        result, percent, message = training_state.parse(
            line=line, verbose=verbose, progress=progress
        )
        print(f"[Training] [{datetime.now().isoformat()}] {line[:-1]}")
        if result:
            yield result
    ...
    if args.tts_backend == "tortoise":
        ...
        try:
            children = [
                ...
                if "./src/train.py" in p.info["cmdline"]
            ]
        except Exception as e:
            pass
...
def whisper_transcribe(file, language=None):
    ...
    if args.whisper_backend == "m-bain/whisperx":
        ...
        result = whisper_model.transcribe(
            file,
            batch_size=args.whisper_batchsize,
            language=language,
...
def validate_waveform(waveform, sample_rate, min_only=False):
    ...
    if duration < MIN_TRAINING_DURATION:
        return "Duration too short ({:.3f}s < {:.3f}s)".format(
            duration, MIN_TRAINING_DURATION
        )

    if not min_only:
        if duration > MAX_TRAINING_DURATION:
            return "Duration too long ({:.3f}s < {:.3f}s)".format(
                MAX_TRAINING_DURATION, duration
            )
...
def transcribe_dataset(voice, language=None, skip_existings=False, progress=None):
    ...
    for file in tqdm(files, desc="Iterating through voice files"):
        ...
        if not quantize_in_memory:
            ...
            try:
                ...
                torchaudio.save(
                    f"{indir}/audio/{basename}", waveform, sample_rate, **kwargs
                )
            except Exception as e:
                print(e)
    ...
    for basename in results:
        try:
            ...
            if len(sanitized["segments"]) > 0 and len(sanitized["segments"]) != len(
                results[basename]["segments"]
            ):
    ...
    for filename in results:
        ...
        for segment in result["segments"]:
            file = filename.replace(
                f".{extension}", f"_{pad(segment['id'], 4)}.{out_extension}"
            )

            sliced, error = slice_waveform(
                waveform,
                sample_rate,
                segment["start"] + start_offset,
                segment["end"] + end_offset,
                trim_silence,
            )
            if error:
            ...
            torchaudio.save(
                f"{indir}/audio/{file}", sliced, TARGET_SAMPLE_RATE, **kwargs
            )
    ...
    def _get_backend(language="en-us", backend="espeak"):
        ...
        if backend == "espeak":
            phonemizer = BACKENDS[backend](
                language, preserve_punctuation=True, with_stress=True
            )
...
def prepare_dataset(
    voice, use_segments=False, text_length=0, audio_length=0, progress=gr.Progress()
):
    indir = f"./training/{voice}/"
    infile = f"{indir}/whisper.json"
    if not os.path.exists(infile):
    ...
    for filename in tqdm(results, desc="Parsing results"):
        ...
        if use_segment and not use_segments:
            ...
            for segment in result["segments"]:
                ...
                if (
                    duration <= MIN_TRAINING_DURATION
                    or MAX_TRAINING_DURATION <= duration
                ):
                    continue

                path = f"{indir}/audio/" + filename.replace(
                    f".{extension}", f"_{pad(segment['id'], 4)}.{out_extension}"
                )
...
def optimize_training_settings(**kwargs):
    ...
    if settings["batch_size"] > lines:
        ...
        messages.append(
            f"Batch size is larger than your dataset, clamping batch size to: {settings['batch_size']}"
        )
    ...
    if settings["batch_size"] / settings["gradient_accumulation_size"] < 2:
        ...
        settings["gradient_accumulation_size"] -= (
            settings["batch_size"] % settings["gradient_accumulation_size"]
        )
        if settings["gradient_accumulation_size"] == 0:
            settings["gradient_accumulation_size"] = 1
    ...
    def get_device_batch_size(vram):
        DEVICE_BATCH_SIZE_MAP = [
            (
                70,
                128,
            ),  # based on an A100-80G, I can safely get a ratio of 4096:32 = 128
            (
                32,
                64,
            ),  # based on my two 6800XTs, I can only really safely get a ratio of 128:2 = 64
            (16, 8),  # based on an A4000, I can do a ratio of 512:64 = 8:1
            (8, 4),  # interpolated
            (
                6,
                2,
            ),  # based on my 2060, it only really lets me have a batch ratio of 2:1
        ]
    ...
    if settings["gpus"] > get_device_count():
        ...
        messages.append(
            f"GPU count exceeds defacto GPU count, clamping to: {settings['gpus']}"
        )
    ...
    if settings["gpus"] <= 1:
        ...
        messages.append(
            f"! EXPERIMENTAL ! Multi-GPU training is extremely particular, expect issues."
        )
    ...
    iterations = calc_iterations(
        epochs=settings["epochs"], lines=lines, batch_size=settings["batch_size"]
    )
...
def save_training_settings(**kwargs):
    ...
    settings["iterations"] = calc_iterations(
        epochs=settings["epochs"], lines=lines, batch_size=settings["batch_size"]
    )

    if not settings["source_model"] or settings["source_model"] == "auto":
        settings["source_model"] = (
            f"./models/tortoise/autoregressive{'_half' if settings['half_p'] else ''}.pth"
        )
    ...
    messages.append(
        f"For {settings['epochs']} epochs with {lines} lines, iterating for {settings['iterations']} steps"
    )

    iterations_per_epoch = settings["iterations"] / settings["epochs"]

    settings["save_rate"] = int(settings["save_rate"] * iterations_per_epoch)
    settings["validation_rate"] = int(
        settings["validation_rate"] * iterations_per_epoch
    )
    ...
    settings["validation_batch_size"] = int(
        settings["batch_size"] / settings["gradient_accumulation_size"]
    )
    ...
    settings["tokenizer_json"] = (
        args.tokenizer_json if args.tokenizer_json else get_tokenizer_jsons()[0]
    )
    ...
    if (
        "learning_rate_scheme" not in settings
        or settings["learning_rate_scheme"] not in LEARNING_RATE_SCHEMES
    ):
        settings["learning_rate_scheme"] = "Multistep"

    settings["learning_rate_scheme"] = LEARNING_RATE_SCHEMES[
    ...
    if settings["learning_rate_scheme"] == "MultiStepLR":
        if not settings["learning_rate_schedule"]:
            ...
            settings["learning_rate_schedule"] = json.loads(
                settings["learning_rate_schedule"]
            )

        settings["learning_rate_schedule"] = schedule_learning_rate(
            iterations_per_epoch, settings["learning_rate_schedule"]
        )

        learning_rate_schema.append(
            f"  gen_lr_steps: {settings['learning_rate_schedule']}"
        )
        ...
        if "learning_rate_period" not in settings:
            settings["learning_rate_period"] = [
                iterations_per_epoch * restart_period for x in range(epochs)
            ]

        settings["learning_rate_restarts"] = [
            iterations_per_epoch * (x + 1) * restart_period for x in range(restarts)
        ]  # [52, 104, 156, 208]

        if "learning_rate_restart_weights" not in settings:
            settings["learning_rate_restart_weights"] = [
                (restarts - x - 1) / restarts for x in range(restarts)
            ]  # [.75, .5, .25, .125]
            settings["learning_rate_restart_weights"][-1] = (
                settings["learning_rate_restart_weights"][-2] * 0.5
            )
        ...
        learning_rate_schema.append(
            f"  eta_min: !!float {settings['learning_rate_min']}"
        )
        learning_rate_schema.append(f"  restarts: {settings['learning_rate_restarts']}")
        learning_rate_schema.append(
            f"  restart_weights: {settings['learning_rate_restart_weights']}"
        )
    ...
    if args.tts_backend == "tortoise":
        use_template(
            f"./models/.template.dlas.yaml",
            f'./training/{settings["voice"]}/train.yaml',
        )
    elif args.tts_backend == "vall-e":
        settings["model_name"] = "[ 'ar-quarter', 'nar-quarter' ]"
        use_template(
            f"./models/.template.valle.yaml",
            f'./training/{settings["voice"]}/config.yaml',
        )
...
def get_valle_models(dir="./training/"):
    return [
        ...
        if os.path.exists(f"{dir}/{d}/config.yaml")
    ]
...
def get_autoregressive_models(dir="./models/finetunes/", prefixed=False, auto=False):
    ...
    for training in os.listdir(f"./training/"):
        if (
            not os.path.isdir(f"./training/{training}/")
            or not os.path.isdir(f"./training/{training}/finetune/")
            or not os.path.isdir(f"./training/{training}/finetune/models/")
        ):
            continue
        models = sorted(
            [
                int(d[:-8])
                for d in os.listdir(f"./training/{training}/finetune/models/")
                if d[-8:] == "_gpt.pth"
            ]
        )
        found = found + [
            f"./training/{training}/finetune/models/{d}_gpt.pth" for d in models
        ]
...
def get_tokenizer_jsons(dir="./models/tokenizers/"):
    additionals = (
        sorted([f"{dir}/{d}" for d in os.listdir(dir) if d[-5:] == ".json"])
        if os.path.isdir(dir)
        else []
    )
    return relative_paths(
        ["./modules/tortoise-tts/tortoise/data/tokenizer.json"] + additionals
    )
...
def tokenize_text(text, config=None, stringed=True, skip_specials=False):
    ...
    decoded = tokenizer.tokenizer.decode(
        encoded, skip_special_tokens=skip_specials
    ).split(" ")
...
def get_dataset_list(dir="./training/"):
    return sorted(
        [
            ...
            and "train.txt" in os.listdir(os.path.join(dir, d))
        ]
    )
...
def get_training_list(dir="./training/"):
    if args.tts_backend == "tortoise":
        return sorted(
            [
                f"./training/{d}/train.yaml"
                for d in os.listdir(dir)
                if os.path.isdir(os.path.join(dir, d))
                and "train.yaml" in os.listdir(os.path.join(dir, d))
            ]
        )
    else:
        return sorted(
            [
                f"./training/{d}/config.yaml"
                for d in os.listdir(dir)
                if os.path.isdir(os.path.join(dir, d))
                and "config.yaml" in os.listdir(os.path.join(dir, d))
            ]
        )
...
def check_for_updates(dir=None):
    ...
    res = curl(
        f"https://{host}/api/v1/repos/{owner}/{repo}/branches/"
    )  # this only works for gitea instances
...
def setup_args(cli=False):
    ...
    parser.add_argument(
        "--share",
        action="store_true",
        default=default_arguments["share"],
        help="Lets Gradio return a public URL to use anywhere",
    )
    parser.add_argument(
        "--listen",
        default=default_arguments["listen"],
        help="Path for Gradio to listen on",
    )
    parser.add_argument(
        "--check-for-updates",
        action="store_true",
        default=default_arguments["check-for-updates"],
    ...
    parser.add_argument(
        "--low-vram",
        action="store_true",
        default=default_arguments["low-vram"],
        help="Disables some optimizations that increases VRAM usage",
    )
    parser.add_argument(
        "--no-embed-output-metadata",
        action="store_false",
    ...
    parser.add_argument(
        "--use-deepspeed",
        action="store_true",
        default=default_arguments["use-deepspeed"],
        help="Use deepspeed for speed bump.",
    )
    parser.add_argument(
        "--use-hifigan",
        action="store_true",
        default=default_arguments["use-hifigan"],
        help="Use Hifigan instead of Diffusion",
    )
    parser.add_argument(
        "--use-rvc",
        action="store_true",
        default=default_arguments["use-rvc"],
        help="Run the outputted audio thorugh RVC",
    )
    parser.add_argument(
        "--rvc-model",
        action="store_true",
        default=default_arguments["rvc-model"],
    ...
    parser.add_argument(
        "--force-cpu-for-conditioning-latents",
        default=default_arguments["force-cpu-for-conditioning-latents"],
        action="store_true",
        help="Forces computing conditional latents to be done on the CPU (if you constantyl OOM on low chunk counts)",
    )
    parser.add_argument(
        "--defer-tts-load",
        default=default_arguments["defer-tts-load"],
        action="store_true",
        help="Defers loading TTS model",
    )
    parser.add_argument(
        "--prune-nonfinal-outputs",
        default=default_arguments["prune-nonfinal-outputs"],
        action="store_true",
        help="Deletes non-final output files on completing a generation",
    )
    parser.add_argument(
        "--device-override",
        default=default_arguments["device-override"],
        help="A device string to override pass through Torch",
    )
    parser.add_argument(
        "--sample-batch-size",
        default=default_arguments["sample-batch-size"],
        type=int,
        help="Sets how many batches to use during the autoregressive samples pass",
    )
    parser.add_argument(
        "--unsqueeze-sample-batches",
        default=default_arguments["unsqueeze-sample-batches"],
        action="store_true",
        help="Unsqueezes sample batches to process one by one after sampling",
    )
    parser.add_argument(
        "--concurrency-count",
        type=int,
        default=default_arguments["concurrency-count"],
    ...
    parser.add_argument(
        "--output-sample-rate",
        type=int,
        default=default_arguments["output-sample-rate"],
        help="Sample rate to resample the output to (from 24KHz)",
    )
    parser.add_argument(
        "--output-volume",
        type=float,
        default=default_arguments["output-volume"],
        help="Adjusts volume of output",
    )
    parser.add_argument(
        "--results-folder",
        type=str,
        default=default_arguments["results-folder"],
    ...
    parser.add_argument(
        "--hf-token",
        type=str,
        default=default_arguments["hf-token"],
        help="HuggingFace Token",
    )
    parser.add_argument(
        "--tts-backend",
        default=default_arguments["tts-backend"],
        help="Specifies which TTS backend to use.",
    )

    parser.add_argument(
        "--autoregressive-model",
        default=default_arguments["autoregressive-model"],
        help="Specifies which autoregressive model to use for sampling.",
    )
    parser.add_argument(
        "--diffusion-model",
        default=default_arguments["diffusion-model"],
        help="Specifies which diffusion model to use for sampling.",
    )
    parser.add_argument(
        "--vocoder-model",
        default=default_arguments["vocoder-model"],
        action="store_true",
        help="Specifies with vocoder to use",
    )
    parser.add_argument(
        "--tokenizer-json",
        default=default_arguments["tokenizer-json"],
        help="Specifies which tokenizer json to use for tokenizing.",
    )

    parser.add_argument(
        "--phonemizer-backend",
        default=default_arguments["phonemizer-backend"],
        help="Specifies which phonemizer backend to use.",
    )

    parser.add_argument(
        "--valle-model",
        default=default_arguments["valle-model"],
        help="Specifies which VALL-E model to use for sampling.",
    )

    parser.add_argument(
        "--whisper-backend",
        default=default_arguments["whisper-backend"],
        action="store_true",
        help="Picks which whisper backend to use (openai/whisper, lightmare/whispercpp)",
    )
    parser.add_argument(
        "--whisper-model",
        default=default_arguments["whisper-model"],
        help="Specifies which whisper model to use for transcription.",
    )
    parser.add_argument(
        "--whisper-batchsize",
        type=int,
        default=default_arguments["whisper-batchsize"],
        help="Specifies batch size for WhisperX",
    )

    parser.add_argument(
        "--training-default-halfp",
        action="store_true",
        default=default_arguments["training-default-halfp"],
        help="Training default: halfp",
    )
    parser.add_argument(
        "--training-default-bnb",
        action="store_true",
        default=default_arguments["training-default-bnb"],
        help="Training default: bnb",
    )

    parser.add_argument(
        "--websocket-listen-port",
        type=int,
        default=default_arguments["websocket-listen-port"],
        help="Websocket server listen port, default: 8069",
    )
    parser.add_argument(
        "--websocket-listen-address",
        default=default_arguments["websocket-listen-address"],
        help="Websocket server listen address, default: 127.0.0.1",
    )
    parser.add_argument(
        "--websocket-enabled",
        action="store_true",
        default=default_arguments["websocket-enabled"],
...
def update_args(**kwargs):
    ...
    args.force_cpu_for_conditioning_latents = settings[
        "force_cpu_for_conditioning_latents"
    ]
    ...
    args.autocalculate_voice_chunk_duration_size = settings[
        "autocalculate_voice_chunk_duration_size"
    ]
...
def deduce_autoregressive_model(voice=None):
    ...
    if voice:
        ...
        if os.path.isdir(dir):
            counts = sorted(
                [int(d[:-8]) for d in os.listdir(dir) if d[-8:] == "_gpt.pth"]
            )
...
def merge_models(
    primary_model_name, secondary_model_name, alpha, progress=gr.Progress()
):
    key_blacklist = []
    ...
    output_path = (
        f"./models/finetunes/{primary_basename}_{secondary_basename}_{suffix}_merge.pth"
    )
